...
(function (W) {
  ...
  W.buildCsvRows = function buildCsvRows(entries) {
    return entries.map(function (e) {
      ...
      return [dateStr, e.clockIn || '', e.clockOut || '', e.breakMinutes || 0, dur != null ? dur : '', e.dayStatus || 'work', e.location || '', (e.description || '').replace(/\r?\n/g, ' '), e.timezone || W.DEFAULT_TIMEZONE];
    });
  };
  W.exportToCsv = function exportToCsv() {
    ...
    const headers = ['Date', 'Clock In', 'Clock Out', 'Break (min)', 'Duration (min)', 'Status', 'Location', 'Description', 'Timezone'];
  ...
  W.exportToJson = function exportToJson() {
    const entries = W.getFilteredEntries().slice().sort(function (a, b) { return (a.date || '').localeCompare(b.date || ''); });
    const data = { profile: W.getProfile(), exportedAt: new Date().toISOString(), entries: entries };
    const json = JSON.stringify(data, null, 2);
    const blob = new Blob([json], { type: 'application/json;charset=utf-8;' });
    const a = document.createElement('a');
    a.href = URL.createObjectURL(blob);
    a.download = 'working-hours-' + W.getProfile().replace(/\s+/g, '-') + '-' + new Date().toISOString().slice(0, 10) + '.json';
    a.click();
    URL.revokeObjectURL(a.href);
  };
})(window.WorkHours);
